...
function CompetenciasController () {
	...
	this.cargarCompetencias = function (data){
		...
		var cantidad = data.competencias.length;
		var idColor = 1;
		...
		for (i = 0; i < cantidad; i++) {
			...
			$(divCompetencia).find('.titulo').text(data.competencias[i].pregunta);
		...
		$.getJSON(server+"/generos",
			function(data){
		    	...
		    	for (i = 0; i < array.length; i++) {
		    		$("#genero").append("<option value='"+array[i].id+"'>"+array[i].nombre+"</option>");
		    	}
		    });
	},
	...
	this.cargarDirectores = function (){
		...
		$.getJSON(server+"/directores",
			function(data){
		    	...
		    	for (i = 0; i < array.length; i++) {
		    		$("#director").append("<option value='"+array[i].id+"'>"+array[i].nombre+"</option>");
		    	}
		    });
	},
		...
		$.getJSON(server+"/actores",
			function(data){
		    	...
		    	// Se recorren y cargan uno/a a uno/a los/as actores/actrices retornados por el backend (data es un array de objetos json)
		    		$("#actor").append("<option value='"+array[i].id+"'>"+array[i].nombre+"</option>");
		    	}
		    });
	},
